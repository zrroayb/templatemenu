        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          ...
          <div className="flex-1 space-y-3">
            <div className="flex items-start justify-between gap-4">
              <h3 className="font-serif text-2xl md:text-3xl font-semibold text-foreground group-hover:text-foreground/90 transition-colors">
                {item.name}
              </h3>
              <motion.div
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {item.tags.map((tag) => (
                  <motion.span
                    ...
                    className="px-3 py-1 rounded-full text-xs font-medium bg-white/60 backdrop-blur-sm text-foreground/60 border border-foreground/10"
                  >
                    {tag}
                  </motion.span>
                ))}
